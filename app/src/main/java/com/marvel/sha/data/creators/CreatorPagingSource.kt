...
internal class CreatorPagingSource(
    private val dao: CreatorDao,
    private val gson: Gson,
    private val load: suspend (Int) -> MarvelResponse,
) : PagingSource<Int, MarvelCreator>() {

    private fun MarvelData.isLastPage() = count == 0

    override fun getRefreshKey(state: PagingState<Int, MarvelCreator>) = state.anchorPosition

    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, MarvelCreator> = try {
    ...
    private suspend fun page(offset: Int): Pair<MarvelData, List<MarvelCreator>> {
        ...
        val entities = dao.paged(offset, offset + PAGE_SIZE - 1).map(gson::fromRoom)
