...
internal fun Map<String, Any>.optString(key: String) = get(key) as? String
...
internal fun Gson.toComicEntity(idx: Int, it: Map<String, Any>) = ComicEntity(
    idx = idx,
    id = it.int("id"),
    data = toJson(it),
)
...
internal fun Gson.fromRoom(it: ComicEntity) = fromJson<MarvelComic>(it.data).apply {
    assert(it.id == id) {
        """
            id mismatch:
            room: $it
